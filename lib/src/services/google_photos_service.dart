...
class GooglePhotosService {
  ...
  bool get isAuthenticated =>
      _currentUser != null && (_authClient != null || _accessToken != null);
      ...
      if (tokenResponse.statusCode == 200) {
        ...
        if (_accessToken != null) {
          ...
          if (returnedScopes != null &&
            ...
            debugPrint(
              '⚠️ ATTENTION: Le scope Google Photos n\'a PAS été accordé par l\'utilisateur sur l\'écran de consentement !',
            );
          }
  ...
  Future<List<PickerAsset>> fetchPickedPhotos(String sessionId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return items
            .map((item) {
              final parsed = _parseMediaItem(item as Map<String, dynamic>);
              if (parsed == null) {
                debugPrint('[GooglePhotosService] Failed to parse item: $item');
              }
              return parsed;
            })
            .whereType<PickerAsset>()
            .toList();
      }
    } catch (e) {
